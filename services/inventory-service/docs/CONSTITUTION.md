...
> tables**." — Azure Architecture Center, _Data considerations for
> microservices_
...
- **Auth transport:** JWT via `passport-jwt`, verified locally in every service.
  Who the caller _is_ comes either from `identity-service` over HTTP or, in services
...
| `inventory-service` | 1 of 8               | 20 / 40                | 0                                | 18    |
...
- **Principle V:** The 2026-09-02 survey recorded no `circulation` specs in
  `inventory-service`. The current service profile below supersedes that
  historical observation for this repository.
...
_This section is specific to this repository. Do not copy it into another
service._

**What it owns:** assets, asset units, circulation (borrow and return),
approvals, and five reference-data lookups. `src/inventory/` holds eight modules.

**What it borrows:** the JWT signature check and the permission guard only
(`platform/auth`, `platform/access-control`). It has no `users` table, no
`auth_sessions` table, and no permission repository.

**What it calls:** `POST {IDENTITY_SERVICE_URL}/auth/introspect` through
`platform/identity/identity.port.ts` and `http-identity.adapter.ts` — **the
platform's reference implementation of Principle II and Principle VIII.** It
caches for `IDENTITY_CACHE_TTL_MS` (default 5000) and returns 503, not 401, when
`identity-service` is unreachable. Both decisions are documented at the adapter and
must not be reversed casually.

**Database posture — Principle VII, fully satisfied.** `DATABASE_URL` names its
own database (`/inventory`), `prisma/inventory.prisma` declares all 15 models in
it, and `prisma/migrations/` exists. The deployment stage runs
`pnpm prisma:deploy`; `start:prod` runs `node dist/src/main.js` and does not
apply migrations implicitly.

**Layering status, measured 2026-09-16:** 8 of 8 modules use the target layered
layout. No production application use case imports a request DTO. The 15-model
ownership matrix passes, and cross-module persistence uses public capability
ports.

**Known deviations:**

- Asset-number generation (`AST-<CAT>/<YEAR>/<NNN>`) is inline in
  `create-asset.use-case.ts` rather than a named domain policy, and its format
  literals are duplicated in `add-units.use-case.ts`.
- The identity cache is per-process and bounded. It uses token digests, TTL
  expiry, concurrent lookup coalescing, and a configurable maximum entry count.
- The approved `20260915100000_approval_consequence_state` migration is applied
  to the configured local database. Each staging or production environment must
  apply it with `pnpm prisma:deploy` before using approval consequence retry
  state there.

**Final verification, measured 2026-09-16:** `pnpm run validate` passed with 71
Jest suites and 407 tests. Static checks found no forbidden layer imports, old
inventory layouts, copied `SUPER_ADMIN` bypass, or cross-owner Prisma access.
