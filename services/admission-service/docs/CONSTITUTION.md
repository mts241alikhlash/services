...
---
# SERVICE PROFILE — admission-service

*This section is specific to this repository. Do not copy it into another
service.*

**What it owns:** PPDB — applicants, applications, waves, announcements,
documents, payments, and applicant notifications. `src/admission/` is currently
**one flat module** holding thirty-four use cases.

**What it borrows:** three platform pieces from `identity-service`
(`platform/auth`, `platform/access-control/permission`, `platform/user`), each
narrowed to a read or a single write.

**What it calls:** `POST {ACADEMIC_SERVICE_URL}/students/enrol` through
`integration/student-enrolment.port.ts` and its HTTP adapter — the platform's
reference **Anti-Corruption Layer**, because it translates admission's applicant
model into academic's `EnrolStudentInput`.

**The saga, and its accepted cost — Principle VIII.** Enrolment used to write
three services' tables in one transaction. Six of the eight tables belong to
`academic-service`, so the transaction moved there and this service now enrols,
then marks its own application `ENROLLED`. **The operation is no longer atomic.**
If the second write fails, a student exists whose application still reads
`ACCEPTED`. That state is visible to the operator and repairable by repeating
the action, because the enrol endpoint is idempotent. Do not restore
`enrollAsStudent()`.

**Database posture — Principle VII, satisfied by owning the database.**
`admission_service`, 9 models, none of them another service's; `prisma:migrate`
and `prisma:deploy` both ship; `start:prod` does not migrate — migration is an
explicit deploy step.

**Layering status:** 0 of 1. This service needs a **submodule split first** —
`applicant`, `application`, `wave`, `announcement`, `document`, `payment`,
`notification` — before layering is worth doing. See `docs/ARCHITECTURE.md`,
stage 1.

**Known deviations:**
- `domain/admission-status.transitions.ts` imports `ConflictException` from
  `@nestjs/common`. This is an **accepted** deviation from Principle I, with the
  reasoning and its limit stated in `docs/ARCHITECTURE.md`.
- `domain/admission.serializers.ts` shapes HTTP responses from the domain layer;
  it moves to `presentation/http/dto/response/` on conversion.
- `prisma-admission-applicant.repository.ts` is 408 lines, over the Principle V
  budget.
