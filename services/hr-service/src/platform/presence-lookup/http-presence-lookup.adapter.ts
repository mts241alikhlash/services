...
@Injectable()
export class HttpAttendancePeriodLookupAdapter extends IAttendancePeriodReadPort {
  constructor(private readonly client: ServiceClient) {
    super()
  }
  ...
  async isClosed(year: number, month: number): Promise<boolean> {
    ...
    if (
      data === null ||
      typeof data !== 'object' ||
      typeof (data as { closed?: unknown }).closed !== 'boolean'
    ) {
      return this.client.malformed(URL_KEY)
    }
    return (data as { closed: boolean }).closed
  }
}
