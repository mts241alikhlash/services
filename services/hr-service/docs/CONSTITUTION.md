...
---
# SERVICE PROFILE — hr-service

*This section is specific to this repository. Do not copy it into another
service.*

**What it owns:** the people who work at the school and what they are paid.
`src/employee/` is the employment record — NIP, NUPTK, hire date, the account
behind the person; `src/reference-data/{employment-type,position,position-category}/`
are the code tables it points at; `src/payroll/{component,assignment,run,payslip}/`
is salary.

`Employee` is an **employment** record, not a teaching one. Who teaches what is
`TeachingAssignment` and `ClassroomSupervisor`, both academic-service's, both
per-semester. **"Guru" is a `Position`** — `isTeachingStaff()` checks
`position.category.code === 'ACADEMIC'`. It was named `Teacher` until
2026-09-11; the rename went through the database, the routes, the permission
codes and every caller.

**What it borrows:** `platform/auth`, `platform/access-control`,
`platform/identity`, `platform/user`, `platform/profile`, `platform/audit-log`,
plus two read slices — `platform/academic-lookup` and `platform/presence-lookup`
- each narrowed to what one screen needs.

**What it calls:** `GET /teaching-assignments/employee-ids` (academic-service,
for the year-filtered roster), and `POST /daily-presences/monthly-summary`,
`POST /daily-presences/by-users`, `GET /presence/periods/:y/:m/closed`
(presence-service, for payroll). All carry `x-provisioning-token` and fail
closed with a 503.

**The independence guard — FR-055/FR-056, and both halves are here.**
`src/employee/domain/repositories/employee-identity-read.port.spec.ts` proves
the roster query applies no position filter;
`src/payroll/payroll-roster-independence.spec.ts` proves nothing under
`src/payroll` reads `position`, `positionCategory`, `employeePosition` or
`employmentType`, or compares against a seeded code. A position the school adds
next year must work the day someone is assigned to it, with no deploy. While
payroll lived in presence-service the two halves sat in repositories that could
not see each other, and deleting either silently halved the rule.

**Database posture — Principle VII, satisfied by owning the database.**
`hr_service`, 10 models, none of them another service's; `prisma:migrate` and
`prisma:deploy` both ship; `start:prod` does not migrate — migration is an
explicit deploy step.

**Layering status:** 4 of 8 submodules. `employee/` and all three of
`reference-data/` are converted. `payroll/{component,assignment,run,payslip}/`
are not — they still carry the original `use-cases/`, `dto/` and `services/`
layout — and `payroll/` is the larger job, with a `shared/` holding steps both
`run` and `payslip` call.

**Known deviations:**
- `prisma-employee.repository.ts` is 324 lines, over the Principle V budget.
- `payroll/run/services/` holds logic that belongs in use cases; it converts
  with the rest of `payroll/`.
