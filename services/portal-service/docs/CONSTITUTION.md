...
---
# SERVICE PROFILE — portal-service

*This section is specific to this repository. Do not copy it into another
service.*

**What it owns:** the public school website — posts, pages, agenda, gallery,
media, taxonomy, homepage. `src/portal/` holds eight modules. It also serves the
built portal SPA via `@nestjs/serve-static` from `PORTAL_DIST_PATH`.

**What it borrows:** four platform pieces from `identity-service` (`platform/auth`,
`platform/access-control`, `platform/audit-log`, `platform/file`), each narrowed
to a read or a single write.

**The public surface.** This is the only service with routes reachable without a
token, which sharpens Principle III and Principle IV:
- Public and admin are **separate controllers with separate response DTOs**
  (`post-public.controller.ts` / `PostDetailDto`, `post.controller.ts` /
  `PostAdminDto`), never one route with an `if` on `request.user`.
- Public routes carry their own throttle configuration
  (`PORTAL_THROTTLE_TTL`, `PORTAL_THROTTLE_LIMIT`), separate from the global
  default.
- Uploads are sniffed with `file-type` rather than trusting the extension or the
  client's `Content-Type`. That check is what stops an uploaded file being
  served back as executable content. Do not add an upload path that bypasses it.

**Database posture — Principle VII, satisfied by owning the database.**
`portal_service`, 15 models, none of them another service's; `prisma:migrate`
and `prisma:deploy` both ship; `start:prod` does not migrate — migration is an
explicit deploy step.

**Layering status:** 0 of 8 modules migrated, but the best-engineered of the
unmigrated services — 36 specs, nothing over 300 lines, and `post`'s repository
already split correctly into `reader` / `writer` / `where` / `includes` /
`mapper` with `post.where.spec.ts` testing query construction in isolation. That
split is the model the other services should copy.

**Known deviations:**
- **Nine files hold between 2 and 9 use case classes each** —
  `manage-agenda.use-cases.ts` alone holds nine. Principle I: split them as
  their own commit before any folder moves.
- `GetAgendaEntriesUseCase` returns `PaginatedResponse<unknown>`. Principle IV
  forbids `unknown` in a declared return type.
