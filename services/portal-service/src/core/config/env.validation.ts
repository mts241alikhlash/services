...
export const envSchema = z
  .object({
    ...
    IDENTITY_CACHE_MAX_ENTRIES: z.coerce
      ...
      .max(100000)
      .default(1000),
    PROFILE_LOOKUP_CACHE_TTL_MS: z.coerce.number().int().min(0).default(60000),
    PROFILE_LOOKUP_CACHE_MAX_ENTRIES: z.coerce
      .number()
      .int()
      .min(1)
      .max(100000)
    ...
    PORTAL_BASE_URL: z
      .string()
      .url('PORTAL_BASE_URL must be a valid URL')
      .optional(),
    PORTAL_DIST_PATH: z.string().min(1).optional(),
    PORTAL_THROTTLE_TTL: z.coerce.number().int().min(1).default(60000),
    PORTAL_THROTTLE_LIMIT: z.coerce.number().int().min(1).default(300),
  })
  .superRefine((env, ctx) => {
