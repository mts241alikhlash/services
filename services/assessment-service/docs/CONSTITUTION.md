...
---
# SERVICE PROFILE — assessment-service

*This section is specific to this repository. Do not copy it into another
service.*

**What it owns:** teaching evaluation. `src/assessment/` is assessment items and
student scores, `src/attendance/` the class register, `src/report-card/` the
report card, and `src/my-dashboard/` a teacher's or student's own slice.

**What it borrows — more than any other service, and that is the point.** Eight
read slices: `platform/academic-lookup`, `platform/enrollment-lookup`,
`platform/employee-identity`, `platform/student-identity`,
`platform/school-unit-identity`, `platform/presence-lookup`,
`platform/profile-lookup`, plus the usual `platform/auth`,
`platform/access-control` and `platform/identity`. A marking screen names a
class, a term, a subject, a teacher and a student, and this service owns none
of those — it owns the mark.

**What it calls:** academic-service for settings, passing scores, teaching
assignments, classrooms, schedules, semesters and years; student-service for
students and enrolments; hr-service for the employee behind a `userId`;
presence-service for daily attendance; identity-service for the report card's
school-unit header. Every one is batched by distinct id where it can be and
fails closed with a 503.

**`GET /dashboards/me` is not dead code.** An earlier note suggesting
`my-dashboard` be deleted was wrong — `assessment-web` calls it, and it is the
widest fan-out in the platform: assessment → academic + hr + student.

**Database posture — Principle VII, satisfied by owning the database.**
`assessment_service`, 6 models, none of them another service's;
`prisma:migrate` and `prisma:deploy` both ship; `start:prod` does not migrate -
migration is an explicit deploy step.

**Layering status:** 4 of 4. Every domain module is converted.

**Known deviations:**
- `prisma-student-score.repository.ts` is 325 lines and
  `prisma-my-dashboard.repository.ts` 303, both over the Principle V budget.
- `http-academic-lookup.adapter.ts` is 382 lines. It is the single adapter for
  eleven academic reads; split it per entity if it grows again.
