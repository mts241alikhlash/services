...
---
# SERVICE PROFILE — presence-service

*This section is specific to this repository. Do not copy it into another
service.*

**What it owns:** two bounded contexts in one deployable. `src/presence/`
(attendance periods, credentials, daily records, devices, leave, scans, work
patterns) records what happened; `src/payroll/` (assignments, components,
payslips, runs) decides what it is worth. Fourteen modules in total.

**What it borrows:** three platform pieces from `identity-service`
(`platform/auth`, `platform/access-control`, `platform/audit-log`).

**What it calls:** `IPayrollRosterPort.listActiveEmployees()`, owned by
`academic-service`. The port exists; its adapter still reads Prisma locally, so
this is the cheapest coupling on the platform to close.

**What it provides:** `academic-service` borrowed `presence/daily-record`,
narrowed to two methods — gate suggestions and a monthly summary. Those two are
public API under Principle IV even though they are currently satisfied by a copy
of the code rather than an HTTP call.

**The split that was reverted — Principle VIII, and the reason it exists.**
Payroll and presence were separated into two services on 2026-08-29 and merged
back on 2026-08-30. The split created an adapter carrying a **copy** of
presence's `summariseMonth` arithmetic, and the copy got four things wrong:
`LATE` counting as a present day, `lateCount` keying on status rather than
minutes, `lateMinutes` accumulating unconditionally, and the month bounded `lte`
rather than `lt`. None failed a build; all four would have shipped payslips that
were quietly, slightly wrong. `payroll/integration/README.md` records it.
...
`presence_service`, 14 models, none of them another service's; `prisma:migrate`
and `prisma:deploy` both ship; `start:prod` does not migrate — migration is an
explicit deploy step.

**Layering status:** 0 of 14 modules migrated, and the **most Prisma leakage on
the platform** (14 files). It also has the most tests (44 specs).

**Known deviations:**
- `CloseAttendancePeriodUseCase` injects `PrismaService` and runs its own query.
  This is the platform's sharpest Principle I violation — fix it first; it is
  one file.
- Thirteen files import Prisma enums into `domain/` or DTOs. Principle I:
  declare the enums in `domain/enums/` and map in the repository.
- `shared/domain/prisma-selects.ts` puts infrastructure vocabulary in a domain
  kernel.
- An approved payroll run cannot be edited (FR-050); that is why closing a
  period refuses while records are incomplete. Do not relax the check to unblock
  a run.
