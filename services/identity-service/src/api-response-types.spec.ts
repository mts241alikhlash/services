...
const NO_CONTENT =
  /@ApiResponse\(\s*\{[^)]*?status:\s*(?:204|302)|@HttpCode\(\s*(?:HttpStatus\.NO_CONTENT|204)\s*\)/s
...
describe('endpoints document what they return', () => {
  ...
  describe('the matcher itself', () => {
    ...
    it('asks nothing of a handler that redirects', () => {
      const redirected = [
        `  @Get('google')`,
      ...
      expect(handlersIn(redirected, 'x.controller.ts')).toEqual([
        { controller: 'x.controller.ts', typed: true },
      ])
    })
