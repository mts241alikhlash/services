...
---
# SERVICE PROFILE — identity-service

*This section is specific to this repository. Do not copy it into another
service.*

**What it owns:** identity for the whole platform — sign-in, sessions, password
lifecycle, users, roles, permissions, audit log. `src/iam/` holds six modules:
`auth`, `user`, `session`, `access-control/permission`, `access-control/role`,
`audit-log`, plus `notification`.

**What it borrows:** nothing. There is no `platform/` directory here and there
should never be one — this *is* the platform.

**What it publishes:** `POST /auth/introspect`, consumed by
`inventory-service`'s `HttpIdentityAdapter`. Under Principle IV that response
DTO is append-only.

**Database posture — Principle VII, satisfied by owning the database.**
`DATABASE_URL` names `identity_service` and nothing else writes there.
`prisma/` declares 20 models across 7 `.prisma` files, none of them another
service's. `prisma/migrations/` exists, `prisma:migrate` and `prisma:deploy`
both ship and are safe to run, and `start:prod` is `node dist/src/main.js` —
migration is an explicit deploy step, not a boot step, so it never races a
second replica.

This read "not satisfied" until 2026-09-09, when the schema was shared and a
migrate from here would have dropped the 108 models it did not declare. The
database was separated rather than the scripts deleted.

**Layering status:** 0 of 6 modules migrated. Conversion order and procedure are
in `docs/ARCHITECTURE.md`; start with `user`.

**Known deviations:**
- `UserEntity` is declared twice under one name — module-local and in
  `shared/domain/entities/`. Resolve when converting `user`.
- `IUserRepository.update` takes `Partial<Pick<UserEntity, ...>>`, which
  Principle IV forbids.
- `permission-codes.constants.ts` is 1759 lines and is exempt from the
  Principle V file budget: it is a catalogue, not logic.
