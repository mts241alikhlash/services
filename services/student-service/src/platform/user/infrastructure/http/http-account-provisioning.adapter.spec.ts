...
describe('HttpAccountProvisioningAdapter', () => {
  ...
  describe('updateProfile', () => {
    it('sends PATCH to /accounts/:userId/profile with an ISO birthDate', async () => {
      ...
      expect(result.birthDate).toEqual(new Date('2010-04-05'))
    })
  })
  ...
  describe('lookup', () => {
    it('sends GET to /accounts/lookup with query params', async () => {
      ...
      const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit]
      expect(url).toBe(
        'https://identity.internal/accounts/lookup?identifier=siswa001&nik=3204014504100002',
      )
  ...
  describe('assignRole', () => {
    it('sends POST to /accounts/:userId/roles', async () => {
      fetchMock.mockResolvedValue({ ok: true, status: 204 })

      await adapter.assignRole('u-1', 'STUDENT')

      const [url, init] = fetchMock.mock.calls[0] as [string, RequestInit]
      expect(url).toBe('https://identity.internal/accounts/u-1/roles')
      expect(init.method).toBe('POST')
      expect(JSON.parse(init.body as string)).toEqual({ roleCode: 'STUDENT' })
    })
  })
})
