...
---
# SERVICE PROFILE — student-service

*This section is specific to this repository. Do not copy it into another
service.*

**What it owns:** the student lifecycle. `src/student/` is the student record
and the student-parent link, `src/parent/` the parent record, `src/enrollment/`
placement into a class for a semester, `src/graduation/` who has left, and
`src/semester-promotion/` the yearly move up.

**What it borrows:** `platform/auth`, `platform/access-control`,
`platform/identity`, `platform/user`, `platform/profile`, plus two read slices -
`platform/academic-lookup` and `platform/report-card-lookup`.

**What it calls:** academic-service for every classroom, semester, academic
year, grade, occupation and education a row is labelled with — batched by
distinct id, one call per entity type per page — `POST /addresses` on
identity-service when enrolling an account that already exists, and
`POST /report-cards/averages` on assessment-service.

**Its five borrowed slices were deleted on 2026-09-09.** `classroom`, `grade`,
`semester`, `academic-year` and `teaching-assignment` are gone, and `prisma/`
went from 26 models to the 6 it owns. Three cross-service reads that earlier
scans had missed turned up in that pass — all relation includes, `orderBy`
through a relation, and a `where` relation filter, never a `prisma.x` call.
**A boundary scan has to read those, not just the model accessor.**

**`student_enrollments` is written by three services.** This service owns it,
and academic-service and assessment-service both `create`, `createMany` and
`update` it. That is not a read to convert; it is an unanswered question about
who owns enrolment, and it has to be answered before those databases can be
called separate in practice.

**Database posture — Principle VII, satisfied by owning the database.**
`student_service`, 6 models, none of them another service's; `prisma:migrate`
and `prisma:deploy` both ship; `start:prod` does not migrate — migration is an
explicit deploy step.

**Layering status:** 5 of 5. Every domain module is converted.

**Known deviations:**
- `enrollment-internal.controller.ts` is 440 lines and
  `prisma-graduation.repository.ts` 388, both over the Principle V budget.
- `http-academic-lookup.adapter.ts` is 332 lines — it is the single adapter for
  every academic read, which is why, and splitting it per entity is the obvious
  move if it grows again.
